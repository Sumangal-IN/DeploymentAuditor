...
public class ExcelReportBuilder {
	...
	private boolean isApplicationInAnomaly(List<ReportCell[]> organizedReportRows) {
		for (ReportCell[] organizedReportRow : organizedReportRows) {
			for (int cell_num = 0; cell_num < organizedReportRow.length; cell_num++)
				if (organizedReportRow[cell_num].isAmbiguous())
					return true;
		}
		return false;
	}
	...
	private void createRowForApplication(ReportCell[] organizedReportRow, int cellnum, Row row) {
		...
		for (int i = 0; i < organizedReportRow.length; i++) {
			cell = row.createCell(++cellnum);
			ExcelReportStyleBuilder.setValueWithFormatting(cell, organizedReportRow[i].getValue(), organizedReportRow[i].isAmbiguous() ? ExcelReportStyleBuilder.bodyStyleAlert : ExcelReportStyleBuilder.bodyStyle);
		}
	}
